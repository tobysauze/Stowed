-- YachtOps Inventory Database Schema
-- Run this migration in your Supabase SQL editor

-- Enable UUID extension
create extension if not exists "uuid-ossp";

-- Enable ltree extension for hierarchical locations (optional but recommended)
...
create table public.item (
  ...
  barcode text,        -- EAN/UPC/QR content
...
create table public.attachment (
  ...
  file_url text not null, -- Supabase Storage or S3 URL
...
create table public.audit_log (
  ...
  entity text not null,            -- 'item' | 'stock' | 'location' | ...
  entity_id bigint not null,
  action text not null,            -- 'create' | 'update' | 'delete' | 'adjust'
  field text,
  old_value text,
  new_value text,
  created_at timestamptz default now()
...
begin
  ...
  v_new := greatest(coalesce(v_old,0) + p_delta, 0); -- Don't allow negative
...
-- For now, allow all authenticated users to read, and write based on role
-- Note: You'll need to adjust these based on your actual auth implementation

-- Category policies
create policy category_read on public.category for select using (true);
create policy category_write on public.category for all using (auth.role() = 'authenticated');

-- Location policies
create policy location_read on public.location for select using (true);
create policy location_write on public.location for all using (auth.role() = 'authenticated');
...
create policy item_update on public.item for update using (auth.role() = 'authenticated');
create policy item_delete on public.item for delete using (auth.role() = 'authenticated');

-- Stock policies
create policy stock_read on public.stock for select using (true);
create policy stock_write on public.stock for all using (auth.role() = 'authenticated');

-- Attachment policies
create policy attachment_read on public.attachment for select using (true);
create policy attachment_write on public.attachment for all using (auth.role() = 'authenticated');

-- Supplier policies
create policy supplier_read on public.supplier for select using (true);
create policy supplier_write on public.supplier for all using (auth.role() = 'authenticated');

-- Item supplier policies
create policy item_supplier_read on public.item_supplier for select using (true);
create policy item_supplier_write on public.item_supplier for all using (auth.role() = 'authenticated');
...
-- Policy: authenticated users can upload, all can read
